...
## docs/GAME_DESIGN.md §2/§9). Default is SELL, matching the design doc's own
## stated default - reversed back to this from an earlier RESERVE-by-default
## pass after direct feedback: a fresh colony that produces and never sells
## anything reads as "gold isn't ticking up," not as a deliberate choice. See
## Routing's class doc.
extends GutTest
...
func test_default_mode_is_sell() -> void:
...
func test_set_mode_emits_routing_changed() -> void:
	...
	Game.routing.set_mode(&"timber", Game.routing.RESERVE)
	assert_signal_emitted_with_parameters(Game.routing, "routing_changed", [&"timber", Game.routing.RESERVE])
...
func test_deliver_sells_for_gold_by_default_and_never_touches_inventory() -> void:
...
func test_routing_choice_is_per_resource() -> void:
	Game.routing.set_mode(&"timber", Game.routing.RESERVE)
	assert_eq(Game.routing.mode_for(&"cod"), Game.routing.SELL, "setting one resource must not affect another")
