...
### Real, per-nation colony names

Direct request: a `colony_names.json` in `docs/` (25 ordered names per nation - Dutch, English, French, Portuguese, Spanish, Swedish) with the instruction that it needs to stay updatable/replaceable/expandable later. 25 is exactly `Balance.max_colonies()` - one name per possible colony slot, index 0 (the Capital) through 24.

- **Kept as plain JSON, deliberately, not converted to a `Resource`/`.tres`** like every other content type in `Db` - the one exception in the project, on purpose. This is exactly the kind of content meant to be hand-edited/expanded later without touching Godot or GDScript at all, and a JSON array of strings is far more approachable for that than `.tres` syntax or the Godot editor's inspector. `Db.COLONY_NAMES_PATH` (`res://data/colony_names.json`) is the copy actually loaded at runtime, loaded and reported through the same `{valid, problems}` shape `_evaluate_directory()` already uses so it shares `_ready()`'s error sweep and `validate()` without a special case; `docs/colony_names.json` is the same file kept there too as the design-reference copy, mirroring how `GAME_DESIGN.md`/`GODOT_PLAN.md` are already mirrored between the Godot and Unity repos this project lives in.
- **`Colony.display_name()`** is the one real entry point for "what does this colony show as," replacing three separate call sites that used to each compute a name from `Db.colony(tier_id).display_name` directly (`ColoniesPanel`'s row list, `MainScreen`'s shipment-delivered toast, `ColonistsPanel`'s assignment buttons). Looks up `Db.colony_name_for(Game.run.nation_id, slot_index)` first; falls back to the old tier-name-plus-cycle-suffix scheme (demoted, not deleted) when no nation was chosen or a run somehow outgrows the list. A colony slot not yet founded (no live `Colony` object) needed the same lookup available without one - `ColoniesPanel._display_name(tier_def, slot_index)` mirrors the same two-step lookup, so a "Found" button's preview name matches exactly what the colony will actually be called once founded.
- Verified with real screenshots (a Dutch run's Capital reading "Fort Nassau," its next-to-found slot correctly previewing "Fort Orange" - both in the JSON's actual order) plus GUT tests for `Db.colony_name_for()` (including proving every nation has a name for every one of the 25 possible slots, not just the first few) and `Colony.display_name()`'s three cases (nation chosen, no nation, past the list's end).
